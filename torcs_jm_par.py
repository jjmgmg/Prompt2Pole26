    ...
    def setup_connection(self):
        ...
        while True:
            ...
            try:
                ...
                # Windows deliverable: NO auto-relaunch (the original Linux pkill/torcs/sh
                # calls don't exist here and only spam errors). Just keep waiting for a TORCS
                # SCR race listening on this port.
                print("Esperando servidor SCR en el puerto %d "
                      "(arranca la carrera TORCS con scr_server en ese puerto)..." % self.port)
                time.sleep(0.5)
...
# ---------------------------------------------------------------------------
# Prompt2Pole agent driver (competition deliverable).
#
# Replaces the stock example/modular drivers: the trained v9c SAC policy drives
# the car through the same SCR transport (Client/ServerState/DriverAction above).
# All the agent logic — observation treatment, actor architecture + weights, and
# action transformation — lives in p2p_agent.Prompt2PoleDriver, which reuses the
# prompt2pole package verbatim so behaviour matches the trained agent.
#
# Run (from this folder, with the project .venv active and a TORCS SCR race up):
#     python torcs_jm_par.py            # connects to port 3001
#     python torcs_jm_par.py -p 3001    # explicit port (parsed by Client)
# The checkpoint defaults to models/v9c_975k_frozen/sac_975000_steps.zip; override
# with the P2P_MODEL environment variable.
# ---------------------------------------------------------------------------

def drive_p2p(c, driver):
    '''Fill the DriverAction (c.R.d) from the Prompt2Pole agent for one server tic.'''
    c.R.d.update(driver.act(c.S.d))
    return


if __name__ == "__main__":
    from p2p_agent import Prompt2PoleDriver

    driver = Prompt2PoleDriver()  # checkpoint via P2P_MODEL env var, else frozen v9c
    C = Client()                  # honors -p / -H / -s ... from the command line
    for step in range(C.maxSteps, 0, -1):
        C.get_servers_input()
        if C.so is None:          # server sent ***shutdown*** / ***restart***
            break
        drive_p2p(C, driver)
        C.respond_to_server()
    C.shutdown()
